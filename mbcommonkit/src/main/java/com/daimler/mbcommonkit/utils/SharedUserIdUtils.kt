...
/**
 * Returns a list of [PackageInfo] where the package name contains [sharedUserId] and
 * where the sharedUserId is equal to [sharedUserId].
 */
fun getPackagesWithSharedUserId(context: Context, sharedUserId: String): List<PackageInfo> {
    val packages = context.packageManager.getInstalledPackages(0)
    return packages.filter {
        it.packageName.contains(sharedUserId) && it.sharedUserId == sharedUserId
    }
}
 ...
 */
fun preferencesWithPackageContext(context: Context, packageName: String, settingsName: String) =
