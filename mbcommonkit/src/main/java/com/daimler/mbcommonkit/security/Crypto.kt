...
/**
 * Fascade class used to generate key in local KeyStore depending on the OS and to encrypt/decrypt some date.
 * <p>
 *     <ul>
 *         <li>{@link AesEncryption} for devices with SDK >= marshmallow</li>
 *         <li>{@link RsaEncryption} for devices with SDK < marshmallow</li>
 *     </ul>
 * </p>
 *
 * @author Lars
...
class Crypto(
    ...
    private val encryptionAlgorithm: EncryptionAlgorithm = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
        AesEncryption(keyStore, keepResults)
    } else {
        RsaEncryption(context, keyStore, keepResults)
    }
