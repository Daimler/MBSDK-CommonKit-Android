...
class CollectionsTest {
    ...
    fun testDuplicatesDefault() {
        assertEquals("Wrong detection of duplicates.",
                true, list.duplicates().isEmpty())
    }

    @Test
    fun testDuplicatesSelector() {
        assertEquals("Duplicates not detected.",
                true, list.duplicates { it.toLowerCase() }.size == 1)
        val tmp = ArrayList(list)
        tmp.add("mYString")
        assertEquals("Duplicates not detected.",
                true, tmp.duplicates { it.toLowerCase() }.size == 2)
    }
    ...
    fun testTakeFrom() {
        ...
        assertEquals("Concatenated list does not contain all elements.", true,
